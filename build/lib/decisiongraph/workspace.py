...
class Workspace:
    ...
    def __init__(self, token: str, root_dir: str, seed: bool = False):
        self.token = token
        self.root = root_dir
        was_new = not os.path.isdir(self.root)
        os.makedirs(self.root, exist_ok=True)
        # Seed only when the caller explicitly asks (e.g. user's FIRST/default
        # project on a fresh device). Subsequent named projects start empty —
        # they're for the user's real work, not a tutorial graph.
        if was_new and seed:
            self._maybe_seed()
    ...
    def _maybe_seed(self):
        """Copy storage/_seed/* into this brand-new workspace if a seed exists."""
        seed_root = os.path.join(config.STORAGE_DIR, "_seed")
        if not os.path.isdir(seed_root):
            return
        try:
            for sub in os.listdir(seed_root):
                src = os.path.join(seed_root, sub)
                dst = os.path.join(self.root, sub)
                if os.path.isdir(src):
                    shutil.copytree(src, dst, dirs_exist_ok=True)
                elif os.path.isfile(src):
                    shutil.copy2(src, dst)
            print(f"[workspace] seeded {self.token} from {seed_root}")
        except Exception as e:
            print(f"[workspace] seed copy failed for {self.token}: {e}")
...
class WorkspaceManager:
    ...
    def get(self, token: str, seed: bool = False) -> Workspace:
        if not self._safe(token):
            raise ValueError("invalid workspace token")
        with self._lock:
            ws = self._ws.get(token)
            if ws is None:
                ws = Workspace(token, os.path.join(self.base, token), seed=seed)
                self._ws[token] = ws
