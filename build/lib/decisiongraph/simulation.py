...
class SimulationManager:
    ...
    def _build_report(self, sim: SimulationState, client) -> None:
        ...
        if raw:
            ...
            # Count EVERY persona_reactions[] entry as a sentiment data point
            # (per-round Twitter/Reddit posts), not just the final per-persona
            # verdict. This gives dramatic real-discourse magnitudes:
            # depth=15 with 7 personas can yield ~100+ reactions.
            pr = raw.get("persona_reactions") or []
            if pr:
                pos = sum(1 for r in pr if (r.get("sentiment") or "").lower() == "positive")
                neg = sum(1 for r in pr if (r.get("sentiment") or "").lower() == "negative")
                neu = sum(1 for r in pr if (r.get("sentiment") or "").lower() == "neutral")
                total = pos + neg + neu
                if not total:
                    total = len(pr)
                consensus_score = round(100 * pos / total) if total else 0
                # rebuild by_persona_counts from the round reactions too
                by_persona_counts = {}
                for r in pr:
                    p = r.get("persona") or "unknown"
                    s = (r.get("sentiment") or "neutral").lower()
                    b = by_persona_counts.setdefault(p, {"positive": 0, "negative": 0, "neutral": 0})
                    b[s] = b.get(s, 0) + 1
            else:
                # No per-round data — fall back to MiroFish's collapsed verdict
