...
-- Files whose STRUCTURAL graph is current but whose SEMANTIC nodes are stale
-- (edited since last semantic pass). Lazy-refreshed on the next semantic query.
CREATE TABLE IF NOT EXISTS semantic_stale (
    repo        TEXT NOT NULL,
    path        TEXT NOT NULL,
    reason      TEXT,              -- 'added_symbols' | 'removed_symbols' | 'god_node' | 'edit'
    flagged_at  TEXT,
    UNIQUE(repo, path)
);
CREATE INDEX IF NOT EXISTS idx_semantic_stale_repo ON semantic_stale(repo);
"""
...
def update_files(db_path: str, files: list[dict], repo: str,
                  god_node_top: int = 15) -> dict:
    """Incrementally update the structural graph for a set of EDITED files.

    `files` = [{"path": "...", "text": "<full new content>"}, ...]

    For each file: delete its old symbols/calls/imports/rationales, re-parse
    the new content, re-insert. Then re-resolve call edges repo-wide so new
    cross-file links land. Pure structural — no LLM, ~tens of ms per file.

    Detects "major" edits (new/removed symbols, or a god-node touched) and
    flags those files in `semantic_stale` so a later semantic query can
    lazily refresh just them. Minor edits (body-only) are flagged with
    reason='edit' (lowest priority).

    Returns per-file {added, removed, major, reason} + overall counts.
    """
    from .codebase_ast import parse_file_full
    started = time.time()
    conn = _connect(db_path)
    ...
    # current god-node leaf names (for "touched a chokepoint" detection)
    god_rows = cur.execute(
        "SELECT s.name FROM calls c JOIN symbols s ON c.dst_id = s.id "
        "WHERE c.repo = ? AND c.confidence >= 0.5 "
        "GROUP BY s.id ORDER BY COUNT(*) DESC LIMIT ?",
        (repo, god_node_top)).fetchall()
    god_names = {r["name"] for r in god_rows}
    ...
    for f in files:
        ...
        # old symbol leaf-names in this file
        old = {r["name"] for r in cur.execute(
            "SELECT name FROM symbols WHERE repo=? AND path=?",
            (repo, path)).fetchall()}

        # purge this file's rows
        cur.execute("DELETE FROM calls WHERE repo=? AND src_id IN "
                    "(SELECT id FROM symbols WHERE repo=? AND path=?)",
                    (repo, repo, path))
        cur.execute("DELETE FROM rationales WHERE repo=? AND path=?", (repo, path))
        cur.execute("DELETE FROM inherits WHERE repo=? AND src_id IN "
                    "(SELECT id FROM symbols WHERE repo=? AND path=?)",
                    (repo, repo, path))
        cur.execute("DELETE FROM imports WHERE repo=? AND src_path=?", (repo, path))
        cur.execute("DELETE FROM symbols WHERE repo=? AND path=?", (repo, path))

        # re-parse + re-insert symbols, imports, rationales
        result = parse_file_full(path, text) or {}
        chunks = result.get("chunks") or []
        new = set()
        for ch in chunks:
            leaf = ch["name"].split(".")[-1]
            new.add(leaf)
            cur.execute(
                "INSERT OR IGNORE INTO symbols"
                "(repo, path, name, qualified_name, kind, start_line, end_line) "
                "VALUES (?,?,?,?,?,?,?)",
                (repo, path, leaf, ch["name"], ch["kind"],
                 ch["start_line"], ch["end_line"]))
        for imp in (result.get("imports") or []):
            tmod = _resolve_relative_module(imp.get("target_module") or "", path)
            cur.execute(
                "INSERT INTO imports(src_path, local_name, target_module, "
                "target_name, kind, repo, dst_module) VALUES (?,?,?,?,?,?,?)",
                (path, imp.get("local_name") or "", tmod,
                 imp.get("target_name"), imp.get("kind") or "symbol", repo, tmod))
        for rat in (result.get("rationales") or []):
            line = rat.get("line") or 0
            sid = None
            for ch in chunks:
                if ch["start_line"] <= line <= ch["end_line"]:
                    row = cur.execute(
                        "SELECT id FROM symbols WHERE repo=? AND path=? "
                        "AND qualified_name=?",
                        (repo, path, ch["name"])).fetchone()
                    if row: sid = row["id"]
            cur.execute(
                "INSERT INTO rationales(repo, path, line, tag, text, symbol_id) "
                "VALUES (?,?,?,?,?,?)",
                (repo, path, line, rat["tag"], rat["text"], sid))

        added   = sorted(new - old)
        removed = sorted(old - new)
        touched_god = bool((added or removed) and (new | old) & god_names) \
                      or bool((set(added) | set(removed)) & god_names)
        if added or removed:
            major, reason = True, ("added_symbols" if added else "removed_symbols")
        elif touched_god:
            major, reason = True, "god_node"
        else:
            major, reason = False, "edit"

        # flag semantic-stale
        cur.execute(
            "INSERT OR REPLACE INTO semantic_stale(repo, path, reason, flagged_at) "
            "VALUES (?,?,?,?)",
            (repo, path, reason, time.strftime("%Y-%m-%dT%H:%M:%S")))

        per_file[path] = {"added": added, "removed": removed,
                           "major": major, "reason": reason}

    # Re-resolve call edges repo-wide (cheap) so new symbols get linked as
    # call targets and edited files' calls point at the right places.
    # We re-run the unambiguous + leaf resolution pass on NULL/edited edges.
    cur.execute(
        "UPDATE calls SET dst_id = ("
        "  SELECT id FROM symbols s WHERE s.repo = calls.repo "
        "    AND s.name = calls.dst_name "
        "    AND (SELECT COUNT(*) FROM symbols s2 WHERE s2.repo=calls.repo "
        "         AND s2.name = calls.dst_name) = 1 LIMIT 1), "
        "  confidence = CASE WHEN confidence < 0.7 THEN 0.7 ELSE confidence END "
        "WHERE repo = ? AND dst_id IS NULL", (repo,))

    conn.commit()
    conn.close()

    majors = [p for p, v in per_file.items() if v["major"]]
    return {
        "updated_files":   list(per_file.keys()),
        "major_files":     majors,
        "minor_files":     [p for p in per_file if p not in majors],
        "per_file":        per_file,
        "semantic_refresh_needed": majors,   # caller refreshes these via LLM
        "elapsed_s":       round(time.time() - started, 3),
    }


def list_semantic_stale(db_path: str, repo: Optional[str] = None) -> list[dict]:
    """Files flagged for semantic refresh (edited since last semantic pass)."""
    if not os.path.exists(db_path): return []
    conn = _connect(db_path)
    q = "WHERE repo = ?" if repo else ""
    args = (repo,) if repo else ()
    rows = conn.execute(
        f"SELECT repo, path, reason, flagged_at FROM semantic_stale {q} "
        f"ORDER BY flagged_at DESC", args).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def clear_semantic_stale(db_path: str, paths: list[str], repo: str) -> int:
    """Clear the stale flag for files whose semantic nodes were just refreshed."""
    if not os.path.exists(db_path) or not paths: return 0
    conn = _connect(db_path)
    ph = ",".join("?" for _ in paths)
    cur = conn.execute(
        f"DELETE FROM semantic_stale WHERE repo=? AND path IN ({ph})",
        [repo] + list(paths))
    n = cur.rowcount
