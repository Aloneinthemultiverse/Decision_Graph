...
class DecisionGraph:
    def __init__(self, storage_dir: str = None, embed_model=None):
        ...
        # Shared embedding model injected by the workspace manager (loaded once
        # process-wide). Falls back to loading its own for standalone/CLI use.
        self.embed_model = embed_model if embed_model is not None else SentenceTransformer(config.EMBED_MODEL)
        self.G = None
        ...
        # try loading existing graph from THIS instance's dir (separate concern)
        try:
            self.G, self.communities, self.summaries = load_graph_state(save_dir=self.storage_dir)
